      ...
      for (let index = 0; index < inputFieldOnMapIDNum.length; index++) {
        ...
        if (geoTags[index] === "homeTown") {
          locationPicker = Selector(
            "app-user-geo-location[formControlName='" +
              geoTags[index] +
              "'] mat-form-field"
          );
        } else {
          locationPicker = Selector(
            "app-user-geo-location[formControlName='whereI" +
              geoTags[index] +
              "'] mat-form-field"
          );
        }
        ...
        const inputSearchForLocation = Selector(
          "input[id='mat-input-" + inputFieldOnMapIDNum[index] + "']"
        );
        await t
  ...
  function userProfileInterests() {
    return test(`Should check interests input on user's Profile.`, async t => {
      ...
      for (let index of _.range(22, 36)) {
        ...
        let exchangeKnowledge = Selector(
          "app-topic-group-card #mat-input-" + index
        );
        await t.click(exchangeKnowledge);
        await t
          .pressKey(" down ".repeat(randomNumber) + " enter")
          .click(closeTagButton);
      }
    });
  }
